...
def parse_with_error_handling(line: str, source_file: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    try:
        row = pos_json_line_to_row(line, source_file)
...
def run(argv: List[str] = None) -> None:
    ...
    input_pattern = (
        ...
        or f"gs://{config['gcs']['raw_bucket']}/{config['gcs']['pos_prefix']}*.json"
    )
    output_table = (
    ...
    LOGGER.info("POS batch: %s -> %s", input_pattern, output_table)
    LOGGER.info("Runner (from options): %s", standard_options.runner or "default")

    with beam.Pipeline(options=options) as p:
        lines = p | "ReadPOS" >> beam.io.ReadFromText(input_pattern)
