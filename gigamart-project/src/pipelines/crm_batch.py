...
from src.common.schemas import RAW_CRM_SCHEMA
from src.common.transforms import crm_csv_line_to_row
...
def parse_with_error_handling(line: str, source_file: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    try:
        row = crm_csv_line_to_row(line, source_file)
...
def run(argv: List[str] = None) -> None:
    ...
    input_pattern = (
        ...
        or f"gs://{config['gcs']['raw_bucket']}/{config['gcs']['crm_prefix']}*.csv"
    )
    output_table = (
        known_args.output_table
        or f"{project_id}:{config['bq']['raw_dataset']}.raw_crm_customers"
    )
    dead_letter_table = (
        known_args.dead_letter_table
        or f"{project_id}:{config['bq']['raw_dataset']}.raw_crm_customers_dlq"
    )
    ...
    LOGGER.info("CRM batch: %s -> %s", input_pattern, output_table)
    LOGGER.info("Runner (from options): %s", standard_options.runner or "default")

    with beam.Pipeline(options=options) as p:
        lines = p | "ReadCRM" >> beam.io.ReadFromText(
            input_pattern,
            skip_header_lines=1,
        )
